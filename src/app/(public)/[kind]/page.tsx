...
const kindMap: Record<string, { api: "latest" | "popular" | "movies" | "ongoing"; title: string; subtitle: string }> = {
  latest: { api: "latest", title: "Update Terbaru", subtitle: "Rilis terbaru dari semua server." },
  popular: { api: "popular", title: "Leaderboard", subtitle: "Rekomendasi populer gabungan." },
  movies: { api: "movies", title: "Movies", subtitle: "Film pilihan dari semua server." },
...
function pageHref(kind: string, page: number, type?: string) {
...
export default async function Page({ params, searchParams }: PageProps) {
  ...
  const result = await unifiedList("all", cfg.api, { page: String(page), type: query.type ?? "all" });
  ...
  return <main className="min-h-screen pb-[calc(7rem+env(safe-area-inset-bottom))] pt-28 lg:pb-16">
    ...
    <section className="mx-auto flex max-w-7xl items-center justify-center gap-3 px-4 pb-10 sm:px-5">
      {page > 1 ? <Link href={pageHref(kind, page - 1, query.type)} className="rounded-full border border-sky-300/20 bg-white/[.055] px-5 py-3 text-sm font-black text-white/75 transition hover:border-sky-300/50 hover:text-white">Sebelumnya</Link> : null}
      {hasNext ? <Link href={pageHref(kind, page + 1, query.type)} className="rounded-full bg-sky-400 px-5 py-3 text-sm font-black text-slate-950 shadow-[0_0_28px_rgba(56,189,248,.28)] transition hover:bg-sky-300">Berikutnya</Link> : null}
    </section>
  </main>;
}
