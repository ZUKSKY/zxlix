...
export function HomeHeroSlider({ slides, base }: { slides: DisplayCard[]; base: string }) {
  const safeSlides = useMemo(() => slides.filter((slide) => Boolean(slide?.banner ?? slide?.poster)).slice(0, 10), [slides]);
  ...
  return <section className="idlix-hero relative h-[100svh] min-h-[560px] overflow-hidden bg-[#030711]">
    {safeSlides.map((slide, index) => <div key={`${slide.title}-${index}`} className={`absolute inset-0 transition-opacity duration-700 ${index === active ? "opacity-100" : "opacity-0"}`} aria-hidden={index !== active}>
      <PosterImage src={slide.banner} sources={[slide.poster]} alt={slide.title} priority={index === 0} className="size-full object-cover" />
    </div>)}

    <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_18%,rgba(14,165,233,.30),transparent_24rem),linear-gradient(90deg,#030711_0%,rgba(3,7,17,.94)_31%,rgba(3,7,17,.50)_62%,rgba(3,7,17,.88)_100%)]" />
    <div className="absolute inset-x-0 bottom-0 h-[42%] bg-gradient-to-t from-[#030711] via-[#030711]/84 to-transparent" />
    <div className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-[#030711]/70 to-transparent" />

    <div className="relative z-10 mx-auto flex h-full max-w-7xl items-end px-4 pb-8 pt-28 sm:px-6 lg:pb-12 lg:pt-32">
      <div className="w-full max-w-4xl overflow-visible pb-2 sm:pb-3">
        ...
        <h1 className="max-w-4xl overflow-visible pb-2 text-balance text-4xl font-black uppercase leading-[1.05] tracking-[-.045em] text-white drop-shadow-[0_10px_42px_rgba(0,0,0,.85)] sm:text-5xl lg:text-[3.65rem] xl:text-[4.25rem]">{card?.title ?? "ZXLIX"}</h1>
        <div className="mt-5 flex flex-wrap items-center gap-3 text-sm font-bold text-white/86">
        ...
        <p className="mt-5 max-w-2xl text-sm leading-7 text-white/72 sm:text-base">Streaming anime rasa bioskop: hero full-screen, katalog movie/series, bookmark, continue watching, dan player cepat tanpa nama source mentah.</p>
        ...
        {safeSlides.length > 1 ? <div className="mt-7 flex flex-wrap items-center gap-4">
